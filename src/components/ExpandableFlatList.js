/* Ref: https://github.com/cuiyueshuai/react-native-expandable-section-flatlist
 * ver: 1.1.5
 * Apr 2018
**/
import React, { Component } from "react";
import PropTypes from "prop-types";
import { View, Text, FlatList, ScrollView, LayoutAnimation, TouchableOpacity } from "react-native";

class ExpandableFlatList extends Component {
  constructor(props) {
    super(props);
    this.flatList;
    this.layoutStore = [];
      ...
      <View onLayout={this._itemLayout}>
        ...
        <ScrollView scrollEnabled={false}>
          {memberArr.map((rowItem, rowId) => {
            return <View key={rowId}>{renderRow ? renderRow(rowItem, rowId, index) : null}</View>;
          })}
          {memberArr.length > 0 && renderSectionFooterX
            ? renderSectionFooterX(item, sectionId)
            : null}
        </ScrollView>
      </View>
    );
