...
class SnowIcon extends React.Component {
  constructor(props) {
    ...
    this.flakeOneStages = [];
    this.flakeOneStages.push(
      Animated.timing(this.state.flakeOneTimer, {
        toValue: 1,
    ...
    this.flakeTwoStages = [];
    this.flakeTwoStages.push(
      Animated.timing(this.state.flakeTwoTimer, {
        toValue: 1,
    ...
    this.flakeThreeStages.push(
      Animated.timing(this.state.flakeThreeTimer, {
        toValue: 1,
  ...
  componentDidMount() {
    this.state.flakeOneTimer.addListener(({ value }) => {
      this.flushTransformOne(
        this.flakeOneRef,
        this.state.flakeOneX,
        this.state.flakeOneY,
        this.state.flakeOneRotation,
      );
    });
    ...
    this.state.flakeTwoTimer.addListener(({ value }) => {
      this.flushTransformTwo(
        this.flakeTwoRef,
        this.state.flakeTwoX,
        this.state.flakeTwoY,
        this.state.flakeTwoRotation,
      );
    });
    ...
    this.state.flakeThreeTimer.addListener(({ value }) => {
      this.flushTransformThree(
        this.flakeThreeRef,
        this.state.flakeThreeX,
        this.state.flakeThreeY,
        this.state.flakeThreeRotation,
      );
    });
  ...
  flushTransformOne(ref, x, y, rotation) {
    ...
    transformUtil.transformMutateReturn(matrix, {
      x: -(16 / 25.5) * this.state.centreX,
  ...
  flushTransformTwo(ref, x, y, rotation) {
    ...
    transformUtil.transformMutateReturn(matrix, {
      x: -(0 / 25.5) * this.state.centreX,
      y: 16 / 25.5 * this.state.centreY,
      z: 0,
    });
  ...
  flushTransformThree(ref, x, y, rotation) {
    ...
    transformUtil.transformMutateReturn(matrix, {
      x: 17 / 25.5 * this.state.centreX,
      y: 13 / 25.5 * this.state.centreY,
      z: 0,
    });
