...
class WindCloudIcon extends React.Component {
  constructor(props) {
    ...
    this.bobStages = [];
    this.bobStages.push(
      Animated.timing(this.state.cloudTranslateY, {
  ...
  componentDidMount() {
    this.state.cloudPushTimer.addListener(({ value }) => {
      this.flushTransform(
        this.cloudRef,
        this.state.cloudScale,
        this.state.cloudTranslateX,
    ...
    this.state.windTimer.addListener(({ value }) => {
      this.flushWindTransform(
        [this.windOneRef, this.windTwoRef, this.windThreeRef],
        this.state.windVal,
      );
    });
  ...
  runAnimation() {
    ...
    Animated.parallel([
      ...
      Animated.timing(this.state.cloudPushTimer, {
        toValue: 1,
        duration: this.cloudPushLength,
        useNativeDriver: true,
      }),
      Animated.timing(this.state.windTimer, {
        toValue: 1,
  ...
  flushTransform(ref, scale, translateX, translateY) {
    ...
    transformUtil.transformMutateReturn(matrix, { x: -12 / 25.5 * this.state.centreX, y: 0, z: 0 });

    let xVal = translateX.__getValue();
    let yVal = translateY.__getValue();

    transformUtil.transform(matrix, { x: xVal, y: yVal, z: 0 });
  ...
  flushWindTransform(refs, scaleX) {
    ...
    transformUtil.transformMutateReturn(matrices[0], {
      x: -this.state.centreX,
      y: 11 / 25.5 * this.state.centreY,
      z: 0,
    });

    transformUtil.transformMutateReturn(matrices[1], {
      x: -this.state.centreX,
      y: 4.5 / 25.5 * this.state.centreY,
      z: 0,
    });

    transformUtil.transformMutateReturn(matrices[2], {
      x: -this.state.centreX,
      y: -2 / 25.5 * this.state.centreY,
      z: 0,
    });
        ...
        <Animated.View
          ...
          <Svg viewBox={"0 0 512 512"} height={this.props.size} width={this.props.size}>
            <Path
              ...
              d={
                "M417,166.1c-24-24.5-57.1-38.8-91.7-38.8c-34.6,0-67.7,14.2-91.7,38.8c-52.8,\
        ...
        <Animated.View
          ...
          <Svg viewBox={"0 0 512 512"} height={this.props.size} width={this.props.size}>
            <Path
              fill={this.props.colour}
              d={
                "M16,256h64c8.8,0,16-7.2,16-16s-7.2-16-16-16H16c-8.8,0-16,7.2-16,16S7.2,256,16,256z"
              }
            />
          </Svg>
        </Animated.View>
        ...
        <Animated.View
          ...
          <Svg viewBox={"0 0 512 512"} height={this.props.size} width={this.props.size}>
            <Path
              fill={this.props.colour}
              d={
                "M16,320h94c8.8,0,16-7.2,16-16s-7.2-16-16-16H16c-8.8,0-16,7.2-16,16S7.2,320,16,320z"
              }
            />
          </Svg>
        </Animated.View>
        ...
        <Animated.View
          ...
          <Svg viewBox={"0 0 512 512"} height={this.props.size} width={this.props.size}>
            <Path
              fill={this.props.colour}
              d={
                "M144,352H16c-8.8,0-16,7.2-16,16s7.2,16,16,16h128c8.8,0,16-7.2,16-16S152.8,352,\
							144,352z"
              }
            />
          </Svg>
